"""
GEPA baseline for rule arena (US tax) domain - Direct answer version

    python -m baselines.gepa.gepa_rule_arena \
        --minibatch-size 3 \
        --max-calls 10240 \
        --train-size 50 \
        --val-size 25 \
        --num-threads 25 \
        --log-dir baselines/gepa/logs/rule_arena \
        --num-runs 5 \
        --output-dir baselines/gepa/results \
        --wandb-name rule_arena_gepa_paper \
        --temperature 1.0
"""
...
prompt_prefix = """You are a helpful US taxation consultant. Calculate the income tax owed by the taxpayer step-by-step according to the information provided."""
...
class TaxCalculator(dspy.Signature):
    """Calculate US income tax owed."""

    prompt = dspy.InputField(desc="The tax calculation problem with forms and data")
    tax_calculation = dspy.OutputField(
        desc="The step-by-step tax calculation with final answer"
    )


class TaxCalculatorModule(dspy.Module):
    """Tax calculator module."""

    def __init__(self):
        super().__init__()
        self.generate = dspy.ChainOfThought(TaxCalculator)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="GEPA baseline for rule arena domain")
    ...
    parser.add_argument(
        "--wandb-name",
        type=str,
        default="rule_arena_gepa",
        help="W&B run name (default: rule_arena_gepa)",
    )
...
def main():
    ...
    save_optimized_prompt(optimized_model, args.output_dir, "rule_arena")
    ...
    results_file = Path(args.output_dir) / f"rule_arena_gepa_{args.num_runs}runs.json"
