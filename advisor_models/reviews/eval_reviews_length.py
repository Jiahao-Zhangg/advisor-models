"""Evaluation script for reviews length advisor model.
...
Example usage:
    ...
    python -m advisor_models.reviews.eval_reviews_length \
        --gcp \
        --model_name "reviews_v0_qwen2.5_7b_10ep_hint_length" \
        --dataset_path data/reviews/validation_length.parquet \
...
class ReviewsLengthEvaluator:
    """Evaluator for reviews length advisor model via an OpenAI-compatible endpoint."""
    ...
    def evaluate_dataset(
        ...
        results = []
        person_scores = {person: [] for person in LENGTH_PEOPLE}
    ...
    def print_evaluation_report(
        ...
        print("\n" + "=" * 70)
        print("REVIEWS LENGTH ADVISOR EVALUATION REPORT")
        print("=" * 70)
        ...
        for person in LENGTH_PEOPLE:
            ...
            preferred = LENGTH_CRITERIA[person]
            print(
                f"  {person:8s}: {accuracy:.4f} ({count:3d} examples, prefers {preferred:3d} words)"
            )
...
def run_multi_evaluation(
    evaluator: ReviewsLengthEvaluator,
    dataset_path: str,
    num_runs: int = 5,
    max_examples: Optional[int] = None,
    max_workers: int = 12,
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate reviews length advisor model with GCP/vLLM support"
    )
