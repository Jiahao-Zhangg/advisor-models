...
# Training script for math domain
...
export DATA_DIR="$ADVISOR_MODELS_ROOT/data/math"
...
mkdir -p "$ADVISOR_MODELS_ROOT/ckpts/math"

# Run training
"$ADVISOR_MODELS_ROOT/SkyRL/skyrl-train/.venv/bin/python" -m advisor_models.math.main_math \
  data.train_data="['$DATA_DIR/train_gpt-4o-mini_1000.parquet']" \
  data.val_data="['$DATA_DIR/validation_gpt-4o-mini_500.parquet']" \
  ...
  trainer.run_name="math" \
  trainer.resume_mode=null \
  trainer.ckpt_path="$ADVISOR_MODELS_ROOT/ckpts/math"
